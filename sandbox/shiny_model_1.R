...
trips_temp_rain <- read_csv("data/trips_temp_rain.csv")
...
trips_plus <- trips_temp_rain %>%
  ...
  select("StartDate","TAVG","Distance_Miles","Length_min","rainfall","DayOfWeek","Season","RentalAccessPath","PaymentPlan") %>%  
  rename(Rainfall = rainfall, Temperature = TAVG)
...
select_model_options <- c("Rainfall", "DayOfWeek", "Season", "PaymentPlan", "RentalAccessPath","Temperature")
...
server <- function(input, output) {
  ...
  lm1 <- reactive({lm(reformulate(input$variable_opts,input$model_opts), data = trips_plus)})
